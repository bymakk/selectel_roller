...
Если Python не установлен — скачайте с [python.org](https://www.python.org/downloads/) (выберите 3.10 или новее) и установите с галочкой **«Add Python to PATH»**.
...
> 1. Автоматически создаёт виртуальное окружение `.venv` и устанавливает все зависимости из `requirements.txt`.
> 2. Открывает **мастер настройки** — по шагам спрашивает логин, пароль и ID аккаунта Selectel.
> 3. Сохраняет введённые данные в файл `.env`.
> 4. Начинает работу.
...
Сервисный пользователь — это не ваш личный логин, а специальный API-пользователь.

1. Войдите в [my.selectel.ru](https://my.selectel.ru).
2. Перейдите в раздел **IAM** (верхнее меню) → **Сервисные пользователи**.
3. Нажмите **Создать пользователя**.
4. Введите имя (например, `roller`) и нажмите **Создать**.
5. **Сразу скопируйте пароль** — после закрытия окна увидеть его снова невозможно.
6. Зайдите в созданного пользователя и назначьте роль **Администратор проекта** (или **Operator**) хотя бы в одном проекте.

> Если пароль потерян — зайдите в пользователя и нажмите **Сгенерировать новый пароль**.
...
- Linux: `sudo apt install python3` или `sudo dnf install python3`

### «Не удалось авторизоваться» / «401 Unauthorized»

Проверьте:
1. Правильно ли указаны `SEL_USERNAME`, `SEL_PASSWORD`, `SEL_ACCOUNT_ID` в `.env`.
2. У сервисного пользователя есть роль **Администратор** или **Operator** в проекте.
3. Пароль введён без лишних пробелов; если в пароле есть спецсимволы — оберните значение в двойные кавычки:
   ```ini
